(ns tsumugi.methods.test-ingest-scale
  ...
  Python over the committed seed + wire/fixtures/ingest-scale/orgs.json fixture and embedded verbatim —
...
(deftest ingest-offline-lifts-new-from-fixture
  (let [[nodes ties new-nodes new-ties dropped]
        (is*/ingest-offline "wire/fixtures/ingest-scale"
                            "data/seed-scale-power.kotoba.edn")]
    (is (= 628 (count nodes)))
    (is (= 637 (count ties)))
...
(deftest forage-plan-over-committed-seed
  ...
  (let [p (is*/forage-plan "data/seed-scale-power.kotoba.edn")]
